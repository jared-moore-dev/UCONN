...
struct SubArray kadane(int* numbers, int length){
  ...
  for(int j = 1; j < length; j++){
     ...
     */
     /*add the next element to the prevSubarray*/
      if (prevSubArray.sum > 0){
        prevSubArray.end += 1;
        prevSubArray.sum += numbers[j];
        }
      /*else, the current element is the best subarray*/
      else{
...
int main(int argc, char* argv[]){
  ...
  int numbersLength = 1;
  int* numbers = (int*)malloc(sizeof(int)*numbersLength) /* TODO: Call malloc to create an array with space for numbersLength ints*/;
  int x = 0;
  int i = 0;
  while(scanf("%d", &x) == 1 && x != 0){
	...
	TODO: Call realloc to add space for another integer to the numbers array.
       */
      numbers = realloc(numbers, sizeof(int)*(numbersLength+1));
      numbersLength++;
    }
    numbers[i] = x;
    i++;
